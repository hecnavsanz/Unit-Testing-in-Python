...
class TestSimpletest:
    ...
    def test_simpletest(self, setup_driver):
        ...
        setup_driver.get("http://localhost:10081/web-apps/events-modular/index.html")
        # set window size
        setup_driver.set_window_size(1200, 1200)

        # sleep(2)
        # setup_driver.implicitly_wait(2)
        wait = WebDriverWait(setup_driver, 10)
        element = wait.until(EC.presence_of_element_located((By.ID, "navbarDropdown")))
        ...
        # sleep(4)
        # setup_driver.implicitly_wait(4)
        setup_driver.manage().timeouts().implicitlyWait(2, timeunit.SECONDS);
        # Close the popup window with the JSON event summary
        element = setup_driver.find_element(By.CSS_SELECTOR, "#eventModal .btn")
        element.click()
