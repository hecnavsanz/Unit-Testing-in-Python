...
class TestSimpletest:
    ...
    def test_simpletest(self, setup_driver):
        # Global wait setting
        wait = WebDriverWait(setup_driver, timeout=2)

        # Open /web-apps/events-modular/index.html
        setup_driver.get("http://localhost:10080/web-apps/events-modular/index.html")
        # set window size
        setup_driver.set_window_size(1200, 1200)

        # give some time (although wait should do this for you)
        time.sleep(1)
